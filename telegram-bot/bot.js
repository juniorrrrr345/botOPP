...
bot.on('callback_query', async (query) => {
    ...
    try {
        switch(data) {
            ...
            default:
                if (data.startsWith('submenu_') && !data.startsWith('submenu_text_') && !data.startsWith('submenu_photo_') && !data.startsWith('submenu_name_')) {
...
async function handleAdminCallbacks(query) {
    ...
    switch(data) {
        ...
        default:
            // CORRECTION PRINCIPALE : Gestion des sous-menus
            ...
            } else if (data.startsWith('submenu_name_')) {
                const parts = data.split('_');
                const service = parts[2];
                const submenuId = parts[3];
                userStates[userId] = { action: 'editing_submenu_name', service, submenuId };
                await sendOrEditMessage(
                    chatId, 
                    '✏️ <b>Modifier le nom du sous-menu</b>\n\nEnvoyez le nouveau nom :',
                    { reply_markup: { inline_keyboard: [[{ text: '❌ Annuler', callback_data: `edit_submenu_${service}_${submenuId}` }]] } }
                );
            }
    }
...
// CORRECTION PRINCIPALE : Afficher l'édition d'un sous-menu
...
bot.on('message', async (msg) => {
    ...
    // CORRECTION PRINCIPALE : Gestion de l'édition du texte des sous-menus
    ...
    // CORRECTION PRINCIPALE : Gestion de l'édition du nom des sous-menus
    if (userState.action === 'editing_submenu_name') {
        const field = userState.service === 'liv' ? 'livraisonSubmenus' : 
                     userState.service === 'pos' ? 'postalSubmenus' : 'meetupSubmenus';
        
        const submenu = config[field].find(s => s.id === userState.submenuId);
        if (submenu) {
            submenu.name = msg.text;
            await saveConfig(config);
        }
...
bot.on('photo', async (msg) => {
    ...
    // CORRECTION PRINCIPALE : Gestion de l'édition de photo des sous-menus
